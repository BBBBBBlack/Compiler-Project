...
```mermaid
	...
	node54--> |"unary"| node44
	node54--> |"expr"| node75
	node55("<div>I55:<br>assignment → id = ; ·<br></div>")
	node56("<div>I56:<br>array_assignment → id [ id ·] = expr ; <br></div>")
	node56--> |"]"| node76
	node57("<div>I57:<br>array_assignment → id [ num ·] = expr ; <br></div>")
	node57--> |"]"| node77
	node58("<div>I58:<br>expr → expr ·- term <br>expr → expr ·+ term <br>rel → expr ·> expr <br>rel → expr ·< expr <br>rel → expr ·<= expr <br>rel → expr ·>= expr <br>rel → expr ·<br></div>")
	node58--> |">="| node81
	node58--> |">"| node80
	node58--> |"<="| node79
	node58--> |"-"| node67
	node58--> |"<"| node78
	node58--> |"+"| node66
	node59("<div>I59:<br>array_declaration → ·basics id [ id ] = { array_init } <br>array_declaration → ·basics id [ num ] = { array_init } <br>array_declaration → ·basics id [ num ] <br>basics → ·float <br>basics → ·int <br>if_stmt → ·if ( bool ) stmt_or_block block_tail <br>while_loop → ·while ( bool ) stmt_or_block <br>array_assignment → ·id [ id ] = expr ; <br>array_assignment → ·id [ num ] = expr ; <br>declaration → ·array_declaration ; <br>if_stmt → ·if ( bool stmt_or_block block_tail <br>declaration → ·basics id ; <br>assignment → ·id = ; <br>basics → ·void <br>while_loop → while ( bool ·stmt_or_block <br>while_loop → while ( bool ·) stmt_or_block <br>assignment → ·id = expr ; <br>stmt_or_block → ·block <br>break_stmt → ·break ; <br>stmt → ·assignment <br>stmt → ·declaration <br>stmt → ·array_assignment <br>while_loop → ·while ( bool stmt_or_block <br>block → ·{ declarations stmts } <br>stmt → ·while_loop <br>stmt_or_block → ·stmt <br>block → ·{ declarations stmts <br>stmt → ·if_stmt <br>stmt → ·break_stmt <br></div>")
	node59--> |"{"| node11
	node59--> |"while"| node28
	node59--> |")"| node85
	node59--> |"id"| node27
	node59--> |"if"| node29
	node59--> |"assignment"| node19
	node59--> |"int"| node6
	node59--> |"block"| node83
	node59--> |"basics"| node13
	node59--> |"if_stmt"| node20
	node59--> |"float"| node4
	node59--> |"stmt_or_block"| node82
	node59--> |"while_loop"| node22
	node59--> |"array_assignment"| node30
	node59--> |"void"| node5
	node59--> |"declaration"| node21
	node59--> |"break_stmt"| node23
	node59--> |"array_declaration"| node16
	node59--> |"stmt"| node84
	node59--> |"break"| node25
	node60("<div>I60:<br>join → equality ·<br>equality → equality ·== rel <br>equality → equality ·!= rel <br></div>")
	node60--> |"!="| node87
	node60--> |"=="| node86
	node61("<div>I61:<br>bool → join ·<br>join → join ·&& equality <br></div>")
	node61--> |"&&"| node88
	node62("<div>I62:<br>equality → rel ·<br></div>")
	node63("<div>I63:<br>array_declaration → ·basics id [ id ] = { array_init } <br>array_declaration → ·basics id [ num ] = { array_init } <br>array_declaration → ·basics id [ num ] <br>basics → ·float <br>basics → ·int <br>basics → ·void <br>if_stmt → ·if ( bool ) stmt_or_block block_tail <br>while_loop → ·while ( bool ) stmt_or_block <br>array_assignment → ·id [ id ] = expr ; <br>array_assignment → ·id [ num ] = expr ; <br>if_stmt → ·if ( bool stmt_or_block block_tail <br>declaration → ·basics id ; <br>assignment → ·id = ; <br>declaration → ·array_declaration ; <br>if_stmt → if ( bool ·) stmt_or_block block_tail <br>if_stmt → if ( bool ·stmt_or_block block_tail <br>stmt_or_block → ·block <br>assignment → ·id = expr ; <br>break_stmt → ·break ; <br>stmt → ·assignment <br>stmt → ·declaration <br>stmt → ·array_assignment <br>while_loop → ·while ( bool stmt_or_block <br>block → ·{ declarations stmts } <br>stmt → ·while_loop <br>stmt_or_block → ·stmt <br>block → ·{ declarations stmts <br>stmt → ·if_stmt <br>stmt → ·break_stmt <br></div>")
	node63--> |"{"| node11
	node63--> |"while"| node28
	node63--> |")"| node90
	node63--> |"id"| node27
	node63--> |"if"| node29
	node63--> |"assignment"| node19
	node63--> |"int"| node6
	node63--> |"block"| node83
	node63--> |"basics"| node13
	node63--> |"if_stmt"| node20
	node63--> |"float"| node4
	node63--> |"stmt_or_block"| node89
	node63--> |"while_loop"| node22
	node63--> |"array_assignment"| node30
	node63--> |"void"| node5
	node63--> |"declaration"| node21
	node63--> |"break_stmt"| node23
	node63--> |"array_declaration"| node16
	node63--> |"stmt"| node84
	node63--> |"break"| node25
	node64("<div>I64:<br>array_declaration → basics id [ id ] ·= { array_init } <br></div>")
	node64--> |"="| node91
	node65("<div>I65:<br>array_declaration → basics id [ num ] ·= { array_init } <br>array_declaration → basics id [ num ] ·<br></div>")
	node65--> |"="| node92
